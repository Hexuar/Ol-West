...
# Set rounds
execute store result score #rounds ol_west.value run data get entity @s SelectedItem.components.minecraft:custom_data.ol_west.rounds
...
function ol_west:items/revolver/update_rounds_mainhand with storage ol_west:input

execute if score #rounds ol_west.value matches 0 run return 0

# Load new projectile
item modify entity @s weapon.mainhand ol_west:add_projectile